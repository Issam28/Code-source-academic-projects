...
int estim( struct config conf,int mode )
{
...
// ********************************************************
	...
	int i, j,n1=0,n2=0, ScrQte;
	struct config T[100];
	struct config T2[100];
	int pionB = 0, pionN = 0,cB =0, fB = 0, rB=0, rN=0, fN = 0, cN=0,tB = 0, tN = 0, nB = 0, nN = 0;
	
	static int PionNoirSQTable [64] =
	 {
            0,  0,  0,  0,  0,  0,  0,  0,
         ...
         static int PionBlancSQTable [64] =
        {
            0,  0,  0,  0,  0,  0,  0,  0,
            5, 10, 10,-20,-20, 10, 10,  5,
            5, -5,-10,  0,  0,-10, -5,  5,
         ...
         static  int CavalierNoirSQTable [64] =
        {
        ...
        static  int CavalierBlancSQTable [64] =
        {
            -50,-40,-30,-30,-30,-30,-40,-50,
            -40,-20,  0,  5,  5,  0,-20,-40,
            -30,  5, 10, 15, 15, 10,  5,-30,
       ...
       static int  TourNoirSQTable [64] =
        ...
        static int  TourBlancSQTable [64] =
        {
            0,  0,  0,  5,  5,  0,  0,  0,
           -5,  0,  0,  0,  0,  0,  0, -5,
           -5,  0,  0,  0,  0,  0,  0, -5,
        ...
        static int  ReineNoirSQTable [64] =
        {
        ...
        static int  ReineBlancSQTable [64] =
        {
            -20,-10,-10, -5, -5,-10,-10,-20,
            -10,  0,  5,  0,  0,  0,  0,-10,
            -10,  5,  5,  5,  5,  5,  0,-10,
	...
	// cette estimation prend en compte la mobilité des pieces ie nb de mouvements légales
	//material+mobilité

	// parties : nombre de pièces et occupation du centre

          int k,k2,ii=0,jj=0,ii1=0,jj1=0;

	for (i=0; i<8; i++)
	   for (j=0; j<8; j++) {

  
	   	 if (mode=MAX) {               //cas des blancs
             

		switch (conf.mat[i][j]) {
		   case 'p' : ScrQte=+ PionBlancSQTable[i*8+j]  ;
		  			 pionB++;   
		   		      break;

		   case 'c' : ScrQte=+ CavalierBlancSQTable[i*8+j]   ;
		  			 cB++;   
		   		      break;

		   case 'f' : 	ScrQte=+ FooBlancSQTable[i*8+j]   ;
					   fB++; 
					    break;

		   case 't' : 	ScrQte=+ TourBlancSQTable [i*8+j]  ;
		   				tB++;
		   				 break;

		   case 'n' : 	ScrQte=+ ReineBlancSQTable[i*8+j]   ;
		   				nB++;
		   				break;

		   	case 'r' : ScrQte=+ RoiBlancSQTable[i*8+j]   ;
		   				rB++;
		   				break;
		   ...
		   ScrQte=+ (pionB-pionN)*100 + (cB-cN)*320 + (fB-fN)*330 + (tB-tN)*500 +(nB-nN)*900;
		}
	}   else                          //cas des noirs
	{
		switch (conf.mat[i][j]) {

		   case -'p' : ScrQte=+ PionNoirSQTable[i*8+j]  ;
		  			 pionN++;   
		   		      break;

		   case -'c' : ScrQte=+ CavalierNoirSQTable[i*8+j]   ;
		  			 cN++;   
		   		      break;

		   case -'f' : 	ScrQte=+ FooNoirSQTable[i*8+j]   ;
					   fN++; 
					    break;

		   case -'t' : 	ScrQte=+ TourNoirSQTable [i*8+j]  ;
		   				tN++;
		   				 break;

		   case -'n' : 	ScrQte=+ ReineNoirSQTable[i*8+j]   ;
		   				nN++;
		   				break;

		   case -'r' : 	ScrQte=+ RoiNoirSQTable[i*8+j]   ;
		   				rN++;
		   				break;
   ...
   generer_succ( conf, MAX, T, &n1 );    //calcul le nb de mouvements légals pour les blancs
  generer_succ( conf, MIN, T, &n2 );   //calcul le nb de mouvements légals pour les noirs

   ScrQte += (n1-n2)*10 ;

 // return ( (pionB + cB*6 + tB*8 + nB*20) - (pionN + cN*6 + tN*8 + nN*20) ) * 1.4;
...
int main( int argc, char *argv[] )
{
   ...
   if ( argc == 1 ) 
	hauteur = 3;  // par défaut on fixe la profondeur d'évaluation à 4
   else
